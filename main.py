...
def report_tweet(reported_tweet_id, reported_user_id, list_acc_check, time_report_per_link = 5):
    ...
    while times < time_report_per_link:
        ...
        payloa_flow_1 = {
            "input_flow_data": {
                "requested_variant": f'{{"client_app_id":"3033300","client_location":"search:search_filter_top:result","client_referer":"/search","is_media":true,"is_promoted":false,"report_flow_id":"{report_flow_id}","reported_tweet_id":"{reported_tweet_id}","reported_user_id":"{reported_user_id}","source":"reporttweet"}}',
                "flow_context": {
                    "debug_overrides": {},
                    "start_location": {
                        "location": "search",
        ...
        # Send POST request
        flow_token = None
        url_flow_1 = "https://x.com/i/api/1.1/report/flow.json?flow_name=report-flow"
        response_flow_1 = requests.post(url_flow_1, headers=headers, json=payloa_flow_1)
        
        # Output response
        if response_flow_1.status_code == 200:
            print("Request flow 1 successful!")
            flow_token = response_flow_1.json()["flow_token"]
        else:
            print("Failed to send request.")
            print(f"Status Code: {response_flow_1.status_code}")
            print(response_flow_1.text)
        
        url_flow_2 = "https://x.com/i/api/1.1/report/flow.json"
        payload_flow_2 = {
        ...
        response_flow_2 = requests.post(url_flow_2, headers=headers, json=payload_flow_2)
        if response_flow_2.status_code == 200:
            times += 1
            print(f"Report spam post id {reported_tweet_id} successfully. Số lần report: {times}")
        else:
            print("Failed to send request.")
            print(f"Status Code: {response_flow_2.status_code}")
            print(response_flow_2.text)
